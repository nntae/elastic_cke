...
__device__ uint get_smid_FDTD(void) {
...
__global__ void preemp_SMT_FiniteDifferencesKernel(float *output,
                                        const float *input,
                                        const int dimx,
                                        const int dimy,
                                        const int dimz,
										...
										State *status)
{
	...
	unsigned int SM_id = get_smid_FDTD();
	...
	while (1){
		...
		// for (int iter=0; iter<iter_per_subtask; iter++) {
			...
			// bool validr = true;
			// bool validw = true;
			// const int gtidx = (s_bid / 24) * blockDim.x + threadIdx.x;
			// const int gtidy = (s_bid % 24) * blockDim.y + threadIdx.y;
// //			const int gtidx = blockIdx.x * blockDim.x + threadIdx.x;
// //			const int gtidy = blockIdx.y * blockDim.y + threadIdx.y;
			// const int ltidx = threadIdx.x;
			// const int ltidy = threadIdx.y;
