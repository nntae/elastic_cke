...
int kernel_launch(t_kernel_stub *kstub)
{
  if (kstub->kconf.gridsize.y == 0) kstub->kconf.gridsize.y =1;
  kstub->total_tasks = kstub->kconf.gridsize.x * kstub->kconf.gridsize.y; // Numer of ctas to be execute in a slice
  kstub->kconf.initial_blockID = 0; // Initial block index
...
main(int argc, char *argv[])
{
  ...
  std::string metricName[NUM_METRICS]={"gld_transactions", "gst_transactions", "inst_integer", "flop_count_sp", "flop_count_dp", "flop_count_hp",
                                        "dram_read_transactions", "dram_write_transactions"};

  printf("Usage: %s device_num kid\n", argv[0]);
  ...
  // Get kid
  t_Kernel kid = kid_from_name(argv[2]);
  cudaError_t err;
  ...
  switch (kid){
    ...
    case CCONV:
        ...
        create_stubinfo_with_params(&kstub, deviceId, kid, transfers_s, &preemp_s, (void *)RCONV_stub->params);
        break;
    ...
    case SCEDD:
        ...
        create_stubinfo_with_params(&kstub, deviceId, kid, transfers_s, &preemp_s, (void *)GCEDD_stub->params);
        break;
  ...
  // activity API.
  {
    ...
    CUPTI_CALL(cuptiMetricGetIdFromName(device, metricName[m].c_str(), &metricId));
    ...
    // print metric value, we format based on the value kind
    {
      ...
      case CUPTI_METRIC_VALUE_KIND_DOUBLE:
        printf(" %f ", metricValue.metricValueDouble);
        break;
      case CUPTI_METRIC_VALUE_KIND_UINT64:
        printf(" %llu ", (unsigned long long)metricValue.metricValueUint64);
        break;
      case CUPTI_METRIC_VALUE_KIND_INT64:
        printf(" %lld", (long long) metricValue.metricValueInt64);
        break;
      case CUPTI_METRIC_VALUE_KIND_PERCENT:
        printf(" %f", metricValue.metricValuePercent);
        break;
