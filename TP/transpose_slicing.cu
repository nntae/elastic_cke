...
__global__ void original_copy(float *odata, float *idata, int width, int height)
{
...
__global__ void original_copySharedMem(float *odata, float *idata, int width, int height)
{
...
__global__ void original_transposeNaive(float *odata, float *idata, int width, int height)
{
    int xIndex = blockIdx.x * TILE_DIM + threadIdx.x;
    int yIndex = blockIdx.y * TILE_DIM + threadIdx.y;
...
__global__ void original_transposeCoalesced(float *odata, float *idata, int width, int height)
{
    // Handle to thread block group
    cg::thread_block cta = cg::this_thread_block();
    __shared__ float tile[TILE_DIM][TILE_DIM];
...
__global__ void original_transposeNoBankConflicts(float *odata, float *idata, int width, int height)
{
    // Handle to thread block group
    cg::thread_block cta = cg::this_thread_block();
...
__global__ void original_transposeDiagonal(float *odata, float *idata, int width, int height)
{
    // Handle to thread block group
    cg::thread_block cta = cg::this_thread_block();
    __shared__ float tile[TILE_DIM][TILE_DIM+1];
...
__global__ void original_transposeFineGrained(float *odata, float *idata, int width, int height)
{
    // Handle to thread block group
    cg::thread_block cta = cg::this_thread_block();
...
__global__ void original_transposeCoarseGrained(float *odata, float *idata, int width, int height)
{
    // Handle to thread block group
    cg::thread_block cta = cg::this_thread_block();
    __shared__ float block[TILE_DIM][TILE_DIM+1];
...
__global__ void original_slicing_copy(float *odata, float *idata, int width, int height, int gridDimX, int gridDimY, int init_BlkIdx)
{
...
__global__ void slicing_copySharedMem(float *odata, float *idata, int width, int height, int gridDimX, int gridDimY, int init_BlkIdx)
{
    // Handle to thread block group
    cg::thread_block cta = cg::this_thread_block();
    __shared__ float tile[TILE_DIM][TILE_DIM];
...
__global__ void slicing_transposeNaive(float *odata, float *idata, int width, int height, int gridDimX, int gridDimY, int init_BlkIdx)
{
    int blk_index = blockIdx.x + init_BlkIdx;
    int bx = blk_index % gridDimX; // This is necessary because of the grid linearization
    int by = blk_index / gridDimX;
...
__global__ void slicing_transposeCoalesced(float *odata, float *idata, int width, int height, int gridDimX, int gridDimY, int init_BlkIdx)
{
    // Handle to thread block group
    cg::thread_block cta = cg::this_thread_block();
    __shared__ float tile[TILE_DIM][TILE_DIM];
...
__global__ void slicing_transposeNoBankConflicts(float *odata, float *idata, int width, int height, int gridDimX, int gridDimY, int init_BlkIdx)
{
    // Handle to thread block group
    cg::thread_block cta = cg::this_thread_block();
    __shared__ float tile[TILE_DIM][TILE_DIM+1];
...
__global__ void slicing_transposeDiagonal(float *odata, float *idata, int width, int height, int gridDimX, int gridDimY, int init_BlkIdx)
{
    // Handle to thread block group
    cg::thread_block cta = cg::this_thread_block();
    __shared__ float tile[TILE_DIM][TILE_DIM+1];
    ...
    {
        blockIdx_y = bx;
        blockIdx_x = (bx+by)%gridDimX;
    }
    else
    {
        int bid = bx + gridDimX*by;
        blockIdx_y = bid%gridDimY;
        blockIdx_x = ((bid/gridDimY)+blockIdx_y)%gridDim.x;
    }
...
__global__ void slicing_transposeFineGrained(float *odata, float *idata, int width, int height, int gridDimX, int gridDimY, int init_BlkIdx)
{
    // Handle to thread block group
    cg::thread_block cta = cg::this_thread_block();
    __shared__ float block[TILE_DIM][TILE_DIM+1];
...
__global__ void slicing_transposeCoarseGrained(float *odata, float *idata, int width, int height, int gridDimX, int gridDimY, int init_BlkIdx)
{
    // Handle to thread block group
    cg::thread_block cta = cg::this_thread_block();
    __shared__ float block[TILE_DIM][TILE_DIM+1];
...
launch_original(void *arg)
{
    ...
    for (int k = 0; k<8; k++)
    {
        ...
        {
            case 0:
                kernel = &original_copy;
                kernelName = "simple copy       ";
                break;

            case 1:
                kernel = &original_copySharedMem;
                kernelName = "shared memory copy";
                break;

            case 2:
                kernel = &original_transposeNaive;
                kernelName = "naive             ";
                break;

            case 3:
                kernel = &original_transposeCoalesced;
                kernelName = "coalesced         ";
                break;

            case 4:
                kernel = &original_transposeNoBankConflicts;
                kernelName = "optimized         ";
                break;

            case 5:
                kernel = &original_transposeCoarseGrained;
                kernelName = "coarse-grained    ";
                break;

            case 6:
                kernel = &original_transposeFineGrained;
                kernelName = "fine-grained      ";
                break;

            case 7:
                kernel = &original_transposeDiagonal;
                kernelName = "diagonal          ";
                break;
        }

        // set reference solution
        if (kernel == &original_copy || kernel == &original_copySharedMem)
        {
            gold = h_idata;
        }
        else if (kernel == &original_transposeCoarseGrained || kernel == &original_transposeFineGrained)
        {
...
launch_slicing(void *arg)
{
    ...
    // kernel pointer and descriptor
    void (*kernel)(float *, float *, int, int, int, int, int);
    const char *kernelName;

    // execution configuration parameters
    int gridDimX = size_x/TILE_DIM;
    int gridDimY = size_y/TILE_DIM;
    int blocks = gridDimX*gridDimY;
    int blocksPerLaunch = 8;
    dim3 grid(blocks), threads(TILE_DIM,BLOCK_ROWS);
    ...
    for (int k = 0; k<8; k++)
    {
        ...
        {
            kernel<<<blocks, threads>>>(d_odata, d_idata, size_x, size_y, gridDimX, gridDimY, j);
        }
        if (blocks % blocksPerLaunch != 0) kernel<<<blocks % blocksPerLaunch, threads>>>(d_odata, d_idata, size_x, size_y, gridDimX, gridDimY, blocks-(blocks % blocksPerLaunch));
